...
function CreatePlayer(source, permission_level, money, bank, experience, identifier, license, group, roles)
	local self = {}
	...
	self.experienceDisplayed = false
	...
	rTable.setExperience = function(m)
		if type(m) == "number" then
			local prevExperience = self.experience
			local newExperience = m

			self.experience = m

			if((prevExperience - newExperience) < 0)then
				TriggerClientEvent("es:addedExperience", self.source, math.abs(prevExperience - newExperience), settings.defaultSettings.nativeExperienceSystem)
			else
				TriggerClientEvent("es:removedExperience", self.source, math.abs(prevExperience - newExperience), settings.defaultSettings.nativeExperienceSystem)
			end

			if not settings.defaultSettings.nativeExperienceSystem then
				TriggerClientEvent('es:activateExperience', self.source , self.experience)
			end
		else
			log('ES_ERROR: There seems to be an issue while setting Experience, something else then a number was entered.')
			print('ES_ERROR: There seems to be an issue while setting Experience, something else then a number was entered.')
		end
	end	
	...
	rTable.addExperience = function(m)
		if type(m) == "number" then
			local newExperience = self.experience + m

			self.Experience = newExperience

			TriggerClientEvent("es:addedExperience", self.source, m, settings.defaultSettings.nativeExperienceSystem)
			if not settings.defaultSettings.nativeExperienceSystem then
				TriggerClientEvent('es:activateExperience', self.source , self.experience)
			end
		else
			log('ES_ERROR: There seems to be an issue while adding Experience, a different type then number was trying to be added.')
			print('ES_ERROR: There seems to be an issue while adding Experience, a different type then number was trying to be added.')
		end
	end	
	
	rTable.removeExperience = function(m)
		if type(m) == "number" then
			local newExperience = self.experience - m

			self.experience = newExperience

			TriggerClientEvent("es:removedExperience", self.source, m, settings.defaultSettings.nativeExperienceSystem)
			if not settings.defaultSettings.nativeExperienceSystem then
				TriggerClientEvent('es:activateExperience', self.source , self.experience)
			end
		else
			log('ES_ERROR: There seems to be an issue while removing Experience, a different type then number was trying to be removed.')
			print('ES_ERROR: There seems to be an issue while removing Experience, a different type then number was trying to be removed.')
		end
	end	
	...
	rTable.displayExperience = function(m)
		if type(m) == "number" then	
			if not self.experienceDisplayed then
				if settings.defaultSettings.nativeExperienceSystem then
					TriggerClientEvent("es:displayExperience", self.source, math.floor(m))
				else
					TriggerClientEvent('es:activateExperience', self.source , self.experience)
				end
				
				self.experienceDisplayed = true
			end
		else
			log('ES_ERROR: There seems to be an issue while displaying Experience, a different type then number was trying to be shown.')
			print(Experience)
		end
	end	
