...
function LoadUser(identifier, source, new, licenseNotRequired)
	...
	db.retrieveUser(identifier, function(user)
		if user.license or licenseNotRequired then
			Users[source] = CreatePlayer(source, user.permission_level, user.money, user.bank, user.experience, user.identifier, user.license, user.group, user.roles or "")
...
function LoadLicenseUser(license, source, id)
	...
	db.retrieveLicensedUser(license, function(user)
		Users[source] = CreatePlayer(source, user.permission_level, user.money, user.bank, user.experience, user.identifier, user.license, user.group)
	...
	SetTimeout(60000, function()
		Citizen.CreateThread(function()
			for k,v in pairs(Users)do
				if Users[k] ~= nil then
					db.updateUser(v.get('identifier'), {money = v.getMoney(), bank = v.getBank, experience = v.getExperience()}, function()end)
				end
			end
